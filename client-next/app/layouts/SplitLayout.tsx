...
const SplitLayout: React.FC<SplitLayoutProps> = ({ left, right, className, collapsed = false }) => {
    ...
    return (
        ...
        >
            ...
            <div
                className={cn(
                    "h-full bg-white flex flex-col relative transition-all duration-300 ease-in-out",
                    collapsed ? "w-[60px]" : ""
                )}
                style={!collapsed ? { width: `${leftWidth}%` } : {}}
            >
                {left}
            ...
            {/* Right Pane */}
            <div className="flex-1 h-full bg-gray-50 flex flex-col relative min-w-0 transition-all duration-300 ease-in-out">
                {right}
